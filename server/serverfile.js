...
app.post('/addtocart',function(req,res){
 ...
 app.post('/booking',function(req,res){
    ...
    let result=fs.appendFileSync('./data/booking.json', JSON.stringify(req.body),"utf8")
     fs.appendFileSync('./data/booking.json', "\n","utf8")
     let resultdata=""
     if(!result){
         resultdata={success:true,message:"data insert successfully"}
     }
 ...
 app.post('/showbooking',function(req,res){
    let data=fs.readFileSync('./data/booking.json',"utf8")
    let adddata=[]
    data.split('\n').forEach(d=>{
        adddata.push(d)
    })
